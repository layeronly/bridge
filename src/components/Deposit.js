...
import toIcn from "../assets/images/only.svg"
...
const Deposit = () => {
    ...
    const dataONLY = useBalance({ address: address, token: process.env.REACT_APP_L1_ONLY, watch: true, chainId: Number(process.env.REACT_APP_L1_CHAIN_ID)  })
            ...
            if (!ethValue) {
                ...
                if (!parseFloat(ethValue) > 0) {
                    ...
                    if (sendToken === "ONLY") {
                        ...
                        setLoader(true);
                        var depositTxn2 = await crossChainMessenger.approveERC20(process.env.REACT_APP_L1_ONLY, process.env.REACT_APP_L2_ONLY, daiValue)
                        await depositTxn2.wait()
                        var receiptONLY = await crossChainMessenger.depositERC20( process.env.REACT_APP_L1_ONLY, process.env.REACT_APP_L2_ONLY, daiValue)
                        var getReceiptONLY = await receiptONLY.wait()
                        if (getReceiptONLY) {
                            setLoader(false);
                            setEthValue("")
                        }
    ...
    const handleChange = (e) => {
        ...
        if (sendToken == 'ONLY') {
            if (dataONLY.data?.formatted < e.target.value) {
                setErrorInput("Insufficient ONLY balance.")
            } else {
                setErrorInput("")
            }
            setEthValue(e.target.value)
                        ...
                        <div className='deposit_input_wrap'>
                            <Form>
                                ...
                                <div className='input_icn_wrap'>
                                    {sendToken == "ETH" ? <span className='input_icn'><Ethereum style={{ fontSize: '1.5rem' }}/></span> : sendToken == "ONLY" ? <span className='input_icn'><Image src={toIcn} alt="To icn" fluid /></span> : sendToken == "USDT" ? <span className='input_icn'><Usdt style={{ fontSize: '1.5rem' }}/></span> : <span className='input_icn'><Usdc style={{ fontSize: '1.5rem' }}/></span>}
                                </div>
                            </Form>
                        </div>
                        {errorInput && <small className='text-danger'>{errorInput}</small>}
                        {sendToken == 'ETH' ? address && <p className='wallet_bal mt-2'>Balance: {Number(data?.formatted).toFixed(5)} ETH</p> : sendToken == 'USDT' ? address && <p className='wallet_bal mt-2'>Balance: {Number(dataUSDT.data?.formatted).toFixed(5)} USDT</p> : sendToken == 'ONLY' ?  address && <p className='wallet_bal mt-2'>Balance: {Number(dataONLY.data?.formatted).toFixed(5)} ONLY</p> : address && <p className='wallet_bal mt-2'>Balance: {Number(dataUSDC.data?.formatted).toFixed(5)} USDC</p>}

                    </div>
                    <div className='deposit_details_wrap'>
                        <div className="deposit_details">
                            <p>To</p>
                            <h5><Image src={toIcn} alt="To icn" fluid /> ONLY</h5>
                        </div>
                        <div className='deposit_inner_details'>
                            {sendToken == "ETH" ? <span className='input_icn'> <Ethereum style={{ fontSize: '1.5rem' }}/></span> : sendToken == "ONLY" ? <span className='input_icn'><Image src={toIcn} alt="To icn" fluid /></span> : sendToken == "USDT" ? <span className='input_icn'> <Usdt style={{ fontSize: '1.5rem' }}/></span> : <span className='input_icn'> <Usdc style={{ fontSize: '1.5rem' }}/></span> }  <p> You’ll receive: {ethValue ? ethValue : "0"} {sendToken}</p>
                        </div>
                    </div>
